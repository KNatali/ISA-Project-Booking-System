...
@Component({
  selector: 'app-cottage-addition-items-edit',
  templateUrl: './cottage-addition-items-edit.component.html',
  styleUrls: ['./cottage-addition-items-edit.component.css']
})
export class CottageAdditionItemsEditComponent implements OnInit {
  id: any;
  ...
  addItem() {
    ...
    this.route.params.subscribe(param => {
      this.id = param.id;
      this.additionalItemService.saveCottageAdditionalItem(this.id, this.newItem)
        .subscribe(data => {
  ...
  updateItem() {
    ...
    this.route.params.subscribe(param => {
      this.id = param.id;
      this.additionalItemService.updateCottageAdditionalItem(this.id, this.newItem)
        .subscribe(data => {
          let ref = document.getElementById('cancel2');
          ref?.click();
          this.formValue2.reset();
  ...
  deleteItem(itemId: any) {
    ...
    this.route.params.subscribe(param => {
      this.id = param.id;
      this.additionalItemService.deleteCottageAdditionalItem(this.id, itemId)
        .subscribe(data => {
