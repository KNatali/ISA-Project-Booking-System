...
public class CottageReservationMapper {
	...
	public static CottageReservationDTO convertToDTO(CottageReservation c) {
		...
		CottageReservationDTO dto=new CottageReservationDTO(c.getId(), c.getReservationStart().format(formatter), c.getReservationEnd().format(formatter),cottage,c.getPrice(),c.getMaxPersons(),client,null, items);
		return dto;
	}
}
